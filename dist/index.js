System.register("index", ["api"], function (exports_1, context_1) {
		...
		execute: function () {
			...
			renderSets = async () => {
				try {
					...
					const closeSect3 =
						document.querySelector("#log-sect");
				...
				if (closeSect && openSect && closeSect2) {
					...
					if (row) {
						...
						Object.values(questions).map((q) => {
							...
							let name;
							if (
								q.fun_name.charAt(indexOf - 1) == " "
							) {
								name = q.fun_name.slice(
									0,
									indexOf - 1
								);
							} else {
							...
							const isSolved =
								user.tasks.hasOwnProperty(q.id);
							console.log(q.id);
			...
			showTask = async (question, e) => {
				...
				if (closeSect && openSect) {
					...
					const taskDesc =
						document.querySelector("#taskDesc");
	...
	function ${question.fun_name} {
			...
			topshirish = (e) => {
				...
				if (textArea && resultText) {
					...
					try {
						...
						let name;
						if (
							Question.fun_name.charAt(indexOf - 1) ==
							" "
						) {
							name = Question.fun_name.slice(
								0,
								indexOf - 1
							);
						} else {
			...
			showSignIn = async () => {
				const signIn =
					document.getElementById("show-sign-in");
				if (signIn) signIn.classList.add("active");
				const signUp =
					document.getElementById("show-sign-up");
...
System.register("api", [], function (exports_2, context_2) {
	...
	return {
		...
		execute: function () {
			exports_2(
				"baseURL",
				(baseURL = "https://d073cd501036ffc1.mokky.dev")
			);
			//@ts-ignore
			exports_2(
				"api",
